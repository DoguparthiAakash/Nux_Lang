...
class Vector<n: Nat> {
    func concat<m: Nat>(other: Vector<m>): Vector<n + m> {
...
func divide(a: int, b: Positive): int {
...
# Handle effects
var result = state_handler(0).handle(func() {
    var x = get();
    put(x + 1);
    return get();
});
...
var model = new NeuralNetwork([784, 128, 10]);
