...
use nux::{compile_to_asm, assemble, compile, compile_native};
use nux::vm::NuxVm;
use std::env;
use std::fs;
...
/// `nux build-native <file1.nux> [file2.nux ...] --output <name.o>`
/// Compiles pure Nux source files directly to a native object file.
/// No Python, no intermediate scripts — the Nux compiler handles everything.
fn cmd_build_native(args: &[String]) {
    if args.is_empty() {
        eprintln!("  {} Usage: nux build-native <file.nux> [files...] --output <out.o>", "✕".red().bold());
        process::exit(1);
    }
    ...
    if input_files.is_empty() {
        eprintln!("  {} No .nux files specified.", "✕".red().bold());
        process::exit(1);
    }
    ...
    println!("{} {} {} {}",
        ...
        "◆".bright_blue(),
        "nuxc".white().bold(),
        "─ native compilation ─────────────────".truecolor(80, 80, 80)
    );

    // Concatenate all source files
    let mut combined_source = String::new();
    for file in &input_files {
        println!("{}  {} {}", "│".truecolor(80, 80, 80), "·".truecolor(80, 80, 80), file.truecolor(180, 180, 180));
        match fs::read_to_string(file) {
            Ok(src) => combined_source.push_str(&src),
            Err(e) => {
                eprintln!("{} {} {}  Failed to read {}: {}",
                    "╰─".truecolor(80, 80, 80), "✕".red(), "error".red(), file, e);
                process::exit(1);
            }
        }
    ...
    let first_file = input_files.first().map(String::as_str).unwrap_or("<source>");
    ...
    match compile_native(first_file, &combined_source, &c_out) {
        Ok(()) => {
            println!("{} {} {}  {}",
                ...
                "✦".green(),
                "compiled".green(),
                output_file.white().bold()
            );
        }
        Err(e) => {
            eprintln!("{} {} {}  {}",
                "╰─".truecolor(80, 80, 80), "✕".red(), "error".red(), e.white());
            process::exit(1);
        }
    }
}
