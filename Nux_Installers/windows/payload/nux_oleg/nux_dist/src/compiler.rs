...
impl Parser {
    ...
    fn parse_statement_impl(&mut self, out: &mut String, expect_semi: bool) -> Result<(), CompileError> {
        ...
        match &self.current_token {
             ...
             Token::Star => {
                 self.advance();
                 self.parse_expression(out)?; // The pointer address
                 if self.current_token != Token::Eq {
                     return self.error("Expected = after pointer expression".to_string());
                 }
                 self.advance();
                 self.parse_expression(out)?; // The value to write
                 if expect_semi && self.current_token == Token::SemiColon {
                     self.advance();
                 }
                 out.push_str("OP_PTR_WRITE\n");
                 return Ok(());
             },
    ...
    fn parse_unary(&mut self, out: &mut String) -> Result<Type, CompileError> {
        ...
        if self.current_token == Token::Star {
            self.advance();
            self.parse_unary(out)?; // push pointer address
            out.push_str("OP_PTR_READ\n");
            return Ok(Type::Int); // Pointers return integers conceptually in bytecode for now
        }
