# install_user.ps1 - Nux Language Installer for End-Users
...
# 1. Check for nux.exe
if (-not (Test-Path ".\nux.exe")) {
    Write-Host "Error: nux.exe not found! Please ensure you extracted the entire release zip." -ForegroundColor Red
    exit 1
}

# 2. Remove older versions
Write-Host "`n[1/4] Cleaning up old versions..." -ForegroundColor Yellow
...
# 3. Setup Directories
Write-Host "`n[2/4] Setting up installation directories..." -ForegroundColor Yellow
...
Write-Host "`n[4/4] Setting up File Associations..." -ForegroundColor Yellow
