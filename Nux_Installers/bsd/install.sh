#!/usr/bin/env bash
# ============================================================================
#  Nux Programming Language â€” BSD Installer
...
CHECKSUM_URL="https://github.com/DoguparthiAakash/Nux_Installers/releases/latest/download/nux-bsd.sha256"
...
detect_bsd() { uname -s 2>/dev/null || echo "Unknown"; }
...
verify_checksum() {
    ...
    if command -v curl &>/dev/null; then
        ...
        expected_hash=$(fetch -q -o - "$CHECKSUM_URL" 2>/dev/null | head -1 | awk '{print $1}') || true
    fi
    ...
    local actual_hash
    actual_hash=$(sha256sum "$file" 2>/dev/null | awk '{print $1}') || actual_hash=$(shasum -a 256 "$file" 2>/dev/null | awk '{print $1}')
...
show_menu() {
    ...
    printf "    %b6)%b  Build FreeBSD pkg   Create native package\n" "$GREEN" "$NC"
    printf "    %b0)%b  Exit\n" "$GREEN" "$NC"
    echo ""
...
do_install() {
    ...
    nux_print "Downloading" "$CYAN" "nux-bsd.tar.gz..."
    ...
    if command -v curl &>/dev/null; then
        ...
        fetch -o "$TEMP_TAR" "$DOWNLOAD_URL" 2>/dev/null && download_ok=true
    fi
...
do_update() {
    ...
    local download_ok=false
    if command -v curl &>/dev/null; then
        curl -fSL --connect-timeout 30 -o "$TEMP_TAR" "$DOWNLOAD_URL" 2>/dev/null && download_ok=true
    elif command -v fetch &>/dev/null; then
        fetch -o "$TEMP_TAR" "$DOWNLOAD_URL" 2>/dev/null && download_ok=true
    fi
    if [ "$download_ok" = true ] && verify_checksum "$TEMP_TAR"; then
...
do_build_freebsd_pkg() {
    nux_print "Building" "$MAGENTA" "FreeBSD package..."
    rm -rf pkg_stage
    mkdir -p pkg_stage/usr/local/lib/nux/lib pkg_stage/usr/local/bin
    SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
    [ -d "$SCRIPT_DIR/payload" ] && cp -r "$SCRIPT_DIR/payload/"* "pkg_stage/usr/local/lib/nux/"
    
    cat > pkg_stage/+MANIFEST <<EOF
name: "nux"
version: "$VERSION"
origin: "lang/nux"
prefix: "/usr/local"
comment: "Nux Programming Language"
desc: "An incredibly fast, lightweight, Write-Once-Run-Anywhere programming language."
maintainer: "nux@nuxlang.org"
www: "https://github.com/DoguparthiAakash/Nux_Lang"
categories: ["lang"]
EOF
    cat > pkg_stage/+POST_INSTALL <<'POSTINST'
#!/bin/sh
ln -sf /usr/local/lib/nux/nux /usr/local/bin/nux
chmod 755 /usr/local/bin/nux
POSTINST
    cat > pkg_stage/+PRE_DEINSTALL <<'PRERM'
#!/bin/sh
...
PRERM
    ...
    if command -v pkg &>/dev/null; then
        ...
        printf "       %bâœ”%b  Built %bnux-${VERSION}.pkg%b\n" "$GREEN" "$NC" "$CYAN" "$NC"
    else
        nux_error "pkg command not found. Run this on FreeBSD."
    fi
    rm -rf pkg_stage
}
