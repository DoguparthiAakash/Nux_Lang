    ...
    match command.as_str() {
        ...
        "build" | "compile" => {
            ...
            match result {
                ...
                Err(errors) => {
                    print_temple_errors(&errors, &source, source_path);
                }
            }
        },
        "run" => {
            ...
            let compiled_bytes = match result {
                Ok(b) => b,
                Err(errors) => {
                    print_temple_errors(&errors, &source_content, fname);
                    return;
                }
            };
        ...
        "translate" => {
             ...
             let profile = match profile_str {
                 ...
                 "webwasm" => transpiler::TranspileProfile::WebWASM,
        ...
        "build-native" => {
            if args.len() < 3 {
                println!("Usage: nux build-native <source.nux> [output_binary] [--profile std|embedded|webwasm]");
                return;
            }
            let source_path = &args[2];
...
use crate::high_level::CompileError;

fn print_temple_errors(errors: &[CompileError], source_content: &str, file_name: &str) {
    let quotes = [
        "Be calm, for every error is just a step towards perfection.",
        "Peace comes from within. Do not seek it without.",
        "The mind is everything. What you think you become.",
        "Patience is the key to paradise.",
        "Breathe in, breathe out. The bug is transient.",
        "Do your duty, but do not concern yourself with the results."
    ];
    let quote = quotes[std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_secs() as usize % quotes.len()];
    ...
    eprintln!();
    eprintln!("{}             /\\             {}", orange, reset);
    eprintln!("{}            /  \\            {}", orange, reset);
    eprintln!("{}           /____\\           {}", orange, reset);
    eprintln!("{}           | ॐ  |           {}", orange, reset);
    eprintln!("{}          /|____|\\          {}", orange, reset);
    eprintln!("{}         /________\\         {}", orange, reset);
    eprintln!("{}        ║          ║        {}", orange, reset);
    eprintln!("{}╔═══════╩══════════╩═══════════════════════════════════╗{}", orange, reset);
    eprintln!("{}║                   ॐ  SHANTI  ॐ                       ║{}", gold, reset);
    eprintln!("{}╟──────────────────────────────────────────────────────╢{}", orange, reset);
    eprintln!("{}║ {}{:<52}{} ║{}", orange, gold, quote, orange, reset);
    eprintln!("{}╟──────────────────────────────────────────────────────╢{}", orange, reset);
    ...
    for (idx, e) in errors.iter().enumerate() {
        ...
        for word in e.message.split_whitespace() {
            if current_line.len() + word.len() + 1 > 50 {
                msg_lines.push(current_line.clone());
        ...
        if !current_line.is_empty() { msg_lines.push(current_line); }

        eprintln!("{}║{} 🛕 ERROR #{:<42}{}║{}", orange, red, idx + 1, orange, reset);
        for m in msg_lines {
            eprintln!("{}║{} {:<50}{} ║{}", orange, red, m, orange, reset);
        }
        ...
        if line_num > 0 {
            ...
            eprintln!("{}║{} 📍 At: {}:{}:{} {:<31} {}║{}", orange, gold, file_name, line_num, col_num, "", orange, reset);
            
            // Format the code line to fit inside the box
            let max_code_len = 48;
            let code_display = if line_str.len() > max_code_len {
                format!("{}...", &line_str[..max_code_len - 3])
            } else {
                line_str.to_string()
            };
            
            eprintln!("{}║    {:<48}  ║{}", orange, code_display, reset);
            
            // Caret
            let mut caret = String::new();
            for _ in 0..col_num.saturating_sub(1).min(max_code_len) { caret.push(' '); }
            caret.push('^');
            eprintln!("{}║{}    {:<48}{}  ║{}", orange, red, caret, orange, reset);
        }
